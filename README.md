...
Sistema monolitico de gestao de pacientes para fisioterapeutas, feito com FastAPI, SQLite, SQLAlchemy ORM e templates Jinja2.
...
Na primeira execucao, o SQLite cria automaticamente o arquivo `fisio_pacientes.db` e cadastra o primeiro administrador usando `INITIAL_ADMIN_USERNAME`, `INITIAL_ADMIN_PASSWORD` e `INITIAL_ADMIN_FULL_NAME`. Depois que ja existir algum usuario no banco, essas variaveis nao criam novos usuarios automaticamente.
...
app/
  ...
  database.py          # Engine SQLite, Base SQLAlchemy e sessao por request
